...
For the normal recruitment condition, future recruitment deviations were generated assuming recruitment would continue following the same stochastic process as the final assessment years:
...
### Data Generation  
Five sampling scenarios were tested under each recruitment condition: high survey effort (HSE), intermediate survey effort (ISE), low survey effort (LSE), extra low survey effort (XLSE), and no survey scenario (NSE) (@tbl-sas). The "effort" of the survey was based on the number of sampling grids that would be covered and the approximate resulting uncertainty of the data produced. The number of grids for all scenarios range between 150 (XLSE) to 600 (HSE), the associated CVs for the index of abundance range from 30% - 15%, and the effective sample sizes range from 15 - 60. The CV and effective sample size was assumed to be the same for all years of the survey data. Sampling error (variability in data sampled from the OM) and observation error specified in the EM were the same so there was no misspecification of uncertainty. For all scenarios except the NSE, CPUE and length data from the commercial fishery were no longer included after year 2023 to allow the model to rely entirely on the survey data.
...
```{r}
#| label: tbl-sas
#| tbl-cap: "The observational error used to sample data from the OM for high survey effort (HSE), intermediate survey effort (ISE), low survey effort (LSE), extra low survey effort (XLSE), and no survey effort (NSE). For each survey effort, the number of sampling grids (Grids) that effort would translate to, the resulting index of abundance CV, and effective sample size for length composition data are shown. All sampling effort scenarios were tested under both normal and poor recrutiment conditions."
...
Poor Recruitment: The failing recruitment scenario reveals the critical importance of adequate sampling:

Error distributions become notably wider across all sampling scenarios
The performance gap between HSE and reduced effort scenarios increases substantially
Assessment uncertainty becomes more pronounced, particularly for fishing mortality estimates

The NSE scenario's intermediate performance suggests that in some cases, no survey data may be preferable to very poor quality survey data, possibly due to reduced model complexity or length of a continuous data sources available to the model. However, this is only true under the normal recruitment conditions and once poor recruitment was introduced the model performed far worse than the others in almost every metric. An additional assumption of the NSE model is that the fishery data, particularly the CPUE and size data are accurately characterizing the full population. This is likely untrue, as fishers are knowledgeable about where and when to find larger fish and will likely be able to maintain a healthy catch even if the population is declining. We cannot assume that the fishery is an accurate, unbaised representation of what is actually happening in the population.  

Recruitment Scenario Interactions: Poor recruitment conditions amplify the negative effects of reduced sampling effort, creating a compounding effect where assessments are least reliable when accurate information is most needed.

Management Implications
These results highlight several critical considerations for fishery management:

Survey Design Priority: Maintaining adequate survey effort is essential for reliable stock assessments, particularly for fishing mortality estimates used in management decisions
Recruitment Monitoring: During periods of recruitment failure, enhanced rather than reduced monitoring may be necessary to maintain assessment reliability
Risk Assessment: Managers should account for increased uncertainty in stock status during periods of both poor recruitment and reduced survey effort
Cost-Benefit Analysis: While reduced survey effort may seem economically attractive, the substantial increases in assessment uncertainty could lead to suboptimal management decisions with significant long-term costs

The simulation demonstrates that survey data quality has cascading effects on stock assessment reliability, with implications extending beyond simple parameter estimation to fundamental management capacity during critical periods.