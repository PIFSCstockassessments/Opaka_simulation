...
geom_line(aes(x = year, y = med_re, color = scen), linewidth = 1.2) +
facet_wrap(~ Rec_scenario) + 
labs(x = "Year", y = "Median Absolute Recruitment Relative Error") + 
stockplotr::theme_noaa() +
  scale_colour_manual(
    values = arches_colors,
    name = "Sampling Scenario", 
...
For the normal recruitment condition, we found that the HSE and ISE best estimated the true age-0 recruitment of the population over the time series and the XLSE estimated the true age-0 recruitment the worst, particularly in the last few years (@fig-recmare).  