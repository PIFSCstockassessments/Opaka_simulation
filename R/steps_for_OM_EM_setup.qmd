...
abline(h = 0)
lines(x = 2:75, y = recdevs_df$Value, col = "red", lwd = 2)
# dev.off()
...
#vector of declining mu, declines by 20% over 25 years
mu.vec <- sort(stats::runif(n=20,min = -.33,max=0), decreasing =T)
recdev_sd <- 0.16
...
sub_poor_recdevs <- full_poor_recdevs[1:80,]
...
rep <- SS_output(file.path(main.dir, "HRF_15_yrfwd", "1", "em"))
SS_plots(rep)
...
file.path(main.dir, "sigmaR_tests", "sigR_25"),
file.path(main.dir, "sigmaR_tests", "sigR_75")
))
...
reps <- SSgetoutput(dirvec = c(#file.path(main.dir, "SQ_15_yrfwd", "1", "om"),
file.path(main.dir, "SQ_15_yrfwd", "1", "em"),
file.path(main.dir, "SQ_15_yrfwd", "2", "om"),
file.path(main.dir, "SQ_15_yrfwd", "2", "em"),
file.path(main.dir, "SQ_15_yrfwd", "3", "om"),
file.path(main.dir, "SQ_15_yrfwd", "3", "em"),
file.path(main.dir, "SQ_15_yrfwd", "4", "om"),
file.path(main.dir, "SQ_15_yrfwd", "4", "em"),
file.path(main.dir, "SQ_15_yrfwd", "5", "om"),
file.path(main.dir, "SQ_15_yrfwd", "5", "em")
))
reps_sum <- SSsummarize(reps)
SSplotComparisons(reps_sum, legendlabels = c( "reg_em", "hyperstable_em", "poor_rec"))
SSplotComparisons(reps_sum, legendlabels = c("reg_om", "reg_em", "poorrec_om", "hyperstable_em"), print = T, plotdir = main.dir)

SSplotComparisons(reps_sum, subplots = 11, legendlabels = c("sigmaR=0.25", "sigmaR=0.75"))

SS_plots(reps$replist3)
SS_plots(reps$replist4)

reps_sum$indices %>% filter(Fleet == 3) %>% 
mutate(name = recode(name, replist1 = "reg_em", replist2 = "hyperstable_em", replist3 = "poor_rec_em")) %>%
ggplot(aes(x = Yr, group = name)) +
geom_jitter(aes(y = Obs, color = name), size = 2, width = .2) + 
geom_line(aes(y = Exp, color = name), linewidth = 1.5) +
labs(x = "Year", y = "FRS Index") + 
theme_classic() +
...
em_I <- "HRF_15_yrfwd"
Iteration <- 1
like_dir <- file.path(main.dir, "sigmaR_tests", "sigR_75","likelihood_prof")
dir.create(like_dir)
files <- c("ss3.dat", "control.ss_new", "starter.ss", "forecast.ss", "ss.par", "ss3.exe")
file.copy(from = file.path(main.dir, em_I, Iteration,"em", files), to = like_dir)
