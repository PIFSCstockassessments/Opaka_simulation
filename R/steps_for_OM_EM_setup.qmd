...
new_OM_dir <- file.path(main.dir, "opaka-sim", Iteration, "om_2") #TODO: will need to generalize om_2 somehow
...
om_ems <- list(om = c("om", "om_2"), em = c("em", "em_2"))
re_list <- lapply(om_ems, function(x){
    calculate_re_MO(results_scalar, EM = x, OM = x)
    })
re_it2 <- calculate_re_MO(scale_wide, EM = "em_2", OM = "om_2")
...
geom_line(aes(x = year, y = SpawnBio_re, color = as.factor(iteration), group = iteration), linewidth = 1.2) +
labs(x = "Year", y = "SSB") + 
theme_bw()
