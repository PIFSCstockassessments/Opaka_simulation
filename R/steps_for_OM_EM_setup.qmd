...
mods <- SSgetoutput(dirvec = c(
    ...
    # file.path(main.dir, "01-Fsin-mat-paka", "4", "em"), #model did not converge
...
inds_reduced %>%
ggplot(aes(x = Yr)) + 
geom_point(data = . %>% filter(Model == "o" & Fleet == 1), aes(y = Exp), alpha = .35, color = "#00BFC4") +
geom_point(data = . %>% filter(Model == "e" & Fleet == 1), aes(y = Obs), alpha = .5, color = "#F8766D") +
...
inds_reduced %>%
ggplot(aes(x = Yr)) + 
geom_point(data = . %>% filter(Model == "o" & Fleet == 3), aes(y = Exp), alpha = .75, color = "#00BFC4") +
geom_point(data = . %>% filter(Model == "e" & Fleet == 3), aes(y = Obs), alpha = .5, color = "#F8766D") +
...
scale_linetype_manual(labels = c("EM", "OM"), values = c(1,2)) +
scale_color_manual(values = c("#F8766D", "#00BFC4"), labels = c("EM", "OM")) +
facet_wrap(~iteration) + theme_bw()

SSplotComparisons(mods_sum, subplots = 11)
```
