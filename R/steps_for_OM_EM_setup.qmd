...
```
We also create 25 years of "poor" future recruitment. 
```{r}
## years of poor recruitment in the future
poor_recdevs <- matrix(data = NA, nrow = 25, ncol = 100)
sr_rec75 <- rec$SR_rec[75]
sigmar <- .15
for(i in 1:ncol(new_recdevs)){
    
    Calc_rec = sr_rec75*exp(new_recdevs[75,i]) #calculate new recruitment for year 75
    dec_rec <- seq(Calc_rec, 100.5675, length.out = 11) #getting recruitment down to 40% reduction level
...
get_results_all(
    overwrite_files = T,
    user_scenarios =  all_scenario_names[c(1,2,3,7)],
    filename_prefix = "FRS_HRF_scens"
)   
get_fits_all(
    overwrite_files = T,
    user_scenarios = all_scenario_names[c(1,2,3,7)], #all_scenario_names,
    filename_prefix = "FRS_HRF_scens"
)
...
file.path(main.dir, "LRF_poorrec_25_yrfwd", "2", "om"),
file.path(main.dir, "LRF_poorrec_25_yrfwd", "2", "em")
))
...
std_df <- reps$replist2$timeseries %>%
select(Yr, SpawnBio, Recruit_0, 'obs_cat:_1', 'obs_cat:_2') %>%
 rename(
      Comm_catch = `obs_cat:_1`,
      Noncomm_catch = `obs_cat:_2`
    ) %>%
     pivot_longer(
      cols = c(SpawnBio:Noncomm_catch),
      names_to = "name",
      values_to = "value"
    ) %>% 
    filter(Yr > 0) %>%
    group_by(name) %>%
    mutate(value_standardized = (value /mean(value)) ) %>%
    ungroup()

std_df <- reps$replist2$cpue %>% 
select(Yr, Fleet, Obs) %>%
mutate(name = recode(Fleet,  `1` = "Comm_cpue",
                      `3` = "BFISH_cpue"),
       value = Obs) %>%
    select(Yr, name, value) %>%
    group_by(name) %>%
    mutate(value_standardized = (value /mean(value)) ) %>%
    ungroup() %>% 
    bind_rows(std_df)
...
std_df %>% 
ggplot(aes(x = Yr, y = value_standardized, group = name, color = name)) +
geom_point() + 
geom_line() +
theme_bw()
...
geom_point() + 
geom_line() +
gghighlight(str_detect(name, "SpawnBio|BF")) +
theme_bw()
...
geom_point() + 
geom_line() +
gghighlight(str_detect(name, "SpawnBio|cpue")) +
theme_bw()
...
geom_point() + 
geom_line() +
gghighlight(str_detect(name, "SpawnBio|catch")) +
theme_bw()


reps_sum$SpawnBio %>% 
filter(Yr > 0) %>%
mutate(re = (replist2 - replist1)/replist1) %>%
...
rep <- SS_output(file.path(main.dir, "models", "opaka-om-25-R0_trend"))
SS_plots(rep)
reps <- SSgetoutput(dirvec = c(
file.path(main.dir, "HRF_R0_trend_", "1", "om"),
file.path(main.dir, "HRF_R0_trend_", "1", "em")
))
...
reps_sum <- SSsummarize(reps)
SSplotComparisons(reps_sum, legendlabels = c("HRF_om", "HRF_em", "LRF_om", "LRF_em"))
SSplotComparisons(reps_sum, models = c(1,2), legendlabels = c("om", "em"))
