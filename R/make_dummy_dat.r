##Non-exported functions from ss3sim to make dummy data

make_dummy_dat_catch <- function(fleets, years){
...
make_dummy_dat_cpue <- function(fleets, years, seas){
    dummy_dat_list <- lapply(fleets, function(fleet){
        data.frame("year" = years, 
        "seas" = seas, 
        "index" = fleet, 
...
make_dummy_dat_lencomp <- function(fleets, years, len_bins, nsex = 1) {
  ...
  dummy_dat <- cbind(dummy_dat, dummy_df)
  return(dummy_dat)
}
