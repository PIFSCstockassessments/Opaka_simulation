...
## Checking the original EM estimates the original OM well
...
#change life history in cod-om and cod-em 
ss3sim_base(
  iterations = 1,
...
#change the stock recruitment parameters section in cod-om and cod-em 
ss3sim_base(
  iterations = 1,
...
## Put in F timeseries from original SS model and see how it behaves
rep <- SS_output(dir = file.path(main.dir, "models", "03-modified-og"))
head(rep$timeseries)
...
data_orig <- SS_readdat_3.30(file = file.path(main.dir, "models", "03-modified-og", "data.ss"))
data_orig$styr <- 1
data_orig$endyr <- 75
...
SS_writedat_3.30(data_orig, outfile = file.path(main.dir, "models", "03-modified-og", "data_updated.ss"))
ctl <- SS_readctl_3.30(file = file.path(main.dir, "models", "03-modified-og", "control.ss"), use_datlist = T, datlist = file.path(main.dir, "models", "03-modified-og", "data_updated.ss"))
...
SS_writectl_3.30(ctl, outfile = file.path(main.dir, "models", "03-modified-og", "control.ss"), overwrite = TRUE)
start <- SS_readstarter(file = file.path(main.dir, "models", "03-modified-og", "starter.ss"))
start$datfile <- "data_updated.ss"
SS_writestarter(start, file.path(main.dir, "models", "03-modified-og", "starter.ss"), overwrite = TRUE)

trueF_mods <- SSgetoutput(dirvec = c(
    file.path(main.dir, "08-trueF-trans-cod", "1", "om"),
    file.path(main.dir, "08-trueF-trans-cod", "1", "em"),
    file.path(main.dir, "models", "02-no-size-comp")
))
trueF_mods_sum <- SSsummarize(trueF_mods)
SSplotComparisons(trueF_mods_sum, print = TRUE, legendlabels = c("OM", "EM", "Original"), plotdir = file.path(main.dir, "08-trueF-trans-cod", "1"))
# NOTE: 4/22 Model comparison looks pretty good but SSB scale is different
...
recdevs_mods <- SSgetoutput(dirvec = c(
    #file.path(main.dir, "09-recdevs_ageselex-trans-cod", "1", "om"),
    #file.path(main.dir, "09-recdevs_ageselex-trans-cod", "1", "em"),
...
SSplotComparisons(recdevs_mods_sum, legendlabels = c("Rep_2", "Rep_5", "Original_nosize")) #, print = TRUE, plotdir = file.path(main.dir, "08-trueF-trans-cod", "1"))
...
rep <- SS_output(file.path(main.dir, "models", "03-modified-og"))
rep <- SS_output(file.path(main.dir, "09-recdevs_ageselex-trans-cod", "1", "om"))
SS_plots(rep)
